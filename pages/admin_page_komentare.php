...
<?php if (isset($_SESSION["account"]) && ($_SESSION["account"]["user_type"] == "moderator")): ?><body>
    <nav class="topnav height_of_nav"  id="myTopnav">
            ...
            <a style="background-color: #ddd; color: black;" href="admin_page_komentare.php">Komentáre</a>
    </nav>    
<?php endif; ?>
...
<?php
    $results = mysqli_query($conn, "SELECT reviews.id, reviews.username, reviews.review, reviews.recept_id, reviews.datetime, recepty.nazov_receptu FROM reviews 
    JOIN recepty ON reviews.recept_id = recepty.recept_id ORDER BY datetime");
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($results)) { ?>
                    <tr>
                        ...
                        <?php echo '<td><a href="recipe.php?recipe=' . $row['recept_id'] . '">' . $row['review'] . '</a></td>'; ?>
                        <td><?php echo($row['datetime']) ?></td>
                        <?php echo '<td><a href="recipe.php?recipe=' . $row['recept_id'] . '">' . $row['nazov_receptu'] . '</a></td>'; ?>
                        <td width="110"><a href="../scripts/delete_komentar.php?zmaz=<?php echo$row['id']; ?>" class="btn btn-danger mr-2">Zmazať</a></td>
                    </tr>
                <?php } ?> 
            </tbody>
        </table>         
