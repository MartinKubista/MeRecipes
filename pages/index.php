...
<body>
    ...
    <nav class="topnav" id="myTopnav">
        ...
        <a href="index.php?options=Salaty">Šaláty</a>
        <a href="index.php?options=Exotické jedlá">Exotické jedlá</a>
    ...
    <?php     
      ...
      $count_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM recepty WHERE new_old = 'old'");
      ...
      $total_pages = ceil($total_results / $results_per_page);
        ...
        if (isset($_GET['search'])) {
            ...
            $where_clause1 .= "1=1"; 
            $results = mysqli_query($conn, "SELECT * FROM recepty $where_clause AND new_old = 'old'"); 
            $results1 = mysqli_query($conn, "SELECT * FROM images $where_clause1 "); 
            $count_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM recepty $where_clause AND new_old = 'old'");
            $count_result = mysqli_fetch_assoc($count_query);
            $total_results = $count_result['total'];
            ...
            $count_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM recepty WHERE options = '$o' AND new_old = 'old'");
            $count_result = mysqli_fetch_assoc($count_query);
            $total_results = $count_result['total'];
            ...
            $total_pages = ceil($total_results / $results_per_page);
                ...
                $results = mysqli_query($conn, "SELECT * FROM recepty WHERE new_old = 'old' ORDER BY date_of_create DESC LIMIT $start_index, $results_per_page");
                $count_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM recepty WHERE new_old = 'old'");
                $count_result = mysqli_fetch_assoc($count_query);
                $total_results = $count_result['total'];
